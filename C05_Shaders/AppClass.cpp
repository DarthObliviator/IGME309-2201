...
{
	m_uShaderProgramID = LoadShaders("Shaders//Compliment.vs", "Shaders//Compliment.fs");
...
void AppClass::ProcessKeyboard(sf::Event a_event)
{
	if (a_event.key.code == sf::Keyboard::Key::Escape)//Event says I pressed the Escape key
		...
		Compliment != Compliment;								// sets Complimentary colours to true?				
}
void AppClass::Display(void)
{
	...
	GLuint Complimentary = glGetUniformLocation(m_uShaderProgramID, "CompBool");
	glUniform1i(Complimentary, Compliment);								// should turn the complimentary colours on?
